...
if(isset($_POST['submit'])){
	...
	$user_sql = "SELECT *  FROM `admin` WHERE `username` = '{$input_username}'";
	...
	if(mysqli_num_rows($result_sql) > 0){
		...
		$sql = "INSERT INTO `admin`(`name`, `username`, `email`, `password`, `photo`, `role`) VALUES (
			'{$name}','{$input_username}','{$email}','{$p}','{$file_name}','{$rol}')";
...
<body>
	...
	<div class="container">
		<nav aria-label="breadcrumb" class="mt-3">
			<ol class="breadcrumb">
				<li class="breadcrumb-item active" aria-current="page"><i class="fas fa-user-plus mr-1"></i>Registration Form</li>
				<li class="ml-4"><a href="index.php"><i class="fas fa-tachometer-alt mr-1"></i> Dashboard</a></li>
				<li class="ml-4"><a href="all_admin.php"><i class="fas fa-users mr-1"></i> All Admins</a></li>
			</ol>
		</nav>
		<?php 
		...
		<div class="form-data row">
			<div class="col-sm-8">
				<form style="padding: 40px;margin-top: 15px;" action="<?php echo $_SERVER["PHP_SELF"];?>" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group">
						<input type="submit" name="submit" value="Add Admin" class="btn btn-primary btn-block text-uppercase" style="letter-spacing: 1px; font-weight: 600;">
					</div>
				</form>
			</div>
		</div>
